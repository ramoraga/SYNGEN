...
''' The purpose of this script is to process our data generated by blender and to create our dataset.
    This script goes through each EXR file from an input folder that contains raw data from one class
    and extracts an RGB image and a Depth image. These files are then named and saved according to their 
    output folder. After, the script goes through each mask file and renames them corresponding to their
    RGB and Depth image. After processing, you will have a dataset folder containing three folders with
    rgb images, depth images, and mask files accordingly'''


# This function is used to go through the input folder with raw data, and only 
# process png files. At this time, only mask files should be png file type
# **Note: The input folder should only contain raw data for one class

def save_masks(input_folder, output_folder, prefix="mask"):
...
# This function is used to go through the input folder with raw data, and
# process EXR files to extract their RGB image and Depth-channel image. Both
# files are renamed and saved to their corresponding folder
# **Note: The input folder should only contain raw data for one class
...
def extract_rgbd(input_folder, output_rgb_folder, output_depth_folder):
    ...
    for filename in sorted(os.listdir(input_folder)):
        if filename.endswith(".exr"):
            ...
            # Normalize depth data (scale to 0-255) for visualization or storage
